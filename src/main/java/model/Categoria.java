...
public enum Categoria {
    ...
    public boolean verificarSiPertenece(Cliente cliente)
    {
    	return this.getConsumoMinimo() < cliente.consumo() && cliente.consumo() < this.getConsumoMaximo();
    }
	 ...
	 */
	public static Categoria categorizar(Cliente cliente)
	{
		return Arrays.stream(Categoria.values())
			.filter(categoria -> categoria.verificarSiPertenece(cliente)) 
			.findFirst()
			.get();
	}
	 ...
	 * @param cliente
	 */
	public static void recategorizar(Cliente cliente)
	{
		cliente.setCategoria(Categoria.categorizar(cliente));
	}
}
